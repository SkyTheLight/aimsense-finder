...
export function SetupStep({ setup, onSetupChange, onNext, onBack }: SetupStepProps) {
  ...
  const handleDpiChange = (e: React.ChangeEvent<HTMLInputElement>) => { const val = e.target.value; const value = val === '' ? 0 : Math.max(0, Math.min(SENSITIVITY_LIMITS.maxDPI, Number(val))); onSetupChange({ dpi: value, sensitivity, game, mouseGrip, aimingMechanic }); };
  const handleSensChange = (e: React.ChangeEvent<HTMLInputElement>) => { const val = e.target.value; const value = val === '' ? 0 : Math.max(0, Math.min(SENSITIVITY_LIMITS.max, Number(val))); onSetupChange({ dpi, sensitivity: value, game, mouseGrip, aimingMechanic }); };
  const handleGameChange = (newGame: Game) => onSetupChange({ dpi, sensitivity, game: newGame, mouseGrip, aimingMechanic });
  const handleGripChange = (grip: any) => onSetupChange({ dpi, sensitivity, game, mouseGrip: grip, aimingMechanic });
  const handleMechanicChange = (mechanic: any) => onSetupChange({ dpi, sensitivity, game, mouseGrip, aimingMechanic: mechanic });
