...
export function DesktopPageShell({
  ...
  return (
    <div className="min-h-screen w-full bg-[var(--app-bg)] text-[var(--app-text-primary)]">
      ...
      <div className="relative z-10 mx-auto w-full min-w-[1280px] max-w-[1280px] px-8 py-6">
        ...
        <div className="mt-6 rounded-[36px] border border-[var(--app-border)] bg-[var(--app-surface)] px-8 py-8 shadow-[0_24px_80px_rgba(2,6,23,0.10)]">
            ...
            <div className="mb-8 flex items-end justify-between gap-8">
              <div>
                {backHref && backLabel && (
                  <Link
                    href={backHref}
                    className="mb-5 inline-flex rounded-full border border-[var(--app-border)] bg-[var(--app-surface-soft)] px-3 py-1.5 text-xs font-medium uppercase tracking-[0.16em] text-[var(--app-text-secondary)] transition-colors hover:text-[var(--app-text-primary)]"
                  >
                    {backLabel}
                  </Link>
                )}
                {eyebrow && <div className="text-xs font-medium uppercase tracking-[0.22em] text-[var(--app-accent)]">{eyebrow}</div>}
                <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-[var(--app-text-primary)]">{title}</h1>
                {description && <p className="mt-3 max-w-3xl text-base leading-8 text-[var(--app-text-secondary)]">{description}</p>}
              </div>
            </div>
          )}
